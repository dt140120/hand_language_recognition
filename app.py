...
# hàm normalize các điểm tham số
...
def draw_info_text(image, brect, handedness, hand_sign_text, finger_gesture_text):
    ...
    if hand_sign_text != "":
        ...
        if(info_text == 'Left'):
            info_text = 'TAY.TRAI' + ':' + hand_sign_text
        elif(info_text == 'Right'):
            info_text = 'TAY.PHAI' + ':' + hand_sign_text

    cv.putText(image, info_text, (brect[0] + 5, brect[1] - 4),
               cv.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1, cv.LINE_AA)

    if finger_gesture_text != "":
        cv.putText(image, "CU CHI TAY:" + finger_gesture_text, (10, 60),
...
def draw_info(image, fps, mode, number):
    ...
    mode_string = ['LUU KEYPOINT', 'LUU POINT']
    if 1 <= mode <= 2:
        cv.putText(image, "CHE_DO:" + mode_string[mode - 1], (10, 90),
                   cv.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1,
                   cv.LINE_AA)
        if 0 <= number <= 9:
            cv.putText(image, "SO:" + str(number), (10, 110),
                       cv.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1,
                       cv.LINE_AA)
    return image
